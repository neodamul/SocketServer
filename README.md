...
- ControlServer 기반 서버 등록, healthcheck, route 응답 브로커
- log4net 기반 공통 로깅

### ControlServer 브로커 구조

ControlServer는 TCP 트래픽을 프록시하지 않고 접속할 서버 정보를 알려주는 브로커로 동작합니다.

```text
Client -> ControlServer:5000  ROUTE_REQUEST
Client <- ControlServer       ROUTE_RESPONSE(host, port)
Client -> SocketServer        직접 TCP 연결 유지
```

SocketServer는 설정된 port range에서 사용 가능한 포트를 찾아 바인딩한 뒤 ControlServer에 등록합니다. 이후 30초마다 heartbeat를 보내 현재 접속 수와 CPU/MEM/STORAGE 사용률을 보고합니다.

ControlServer는 서버별 현황을 취합하고 클라이언트 route 요청이 오면 `MostAvailableConnections` 정책으로 적절한 서버를 선택합니다.

```text
availableConnections = maxConnections - currentConnections - reservedConnections
```

ControlServer 이중화를 위해 `peers` 설정과 Control-Control registry sync 메시지도 정의되어 있습니다. 1차 구현은 in-memory registry 기반이며 peer로 `SERVER_REGISTRY_UPSERT`를 전파합니다.

### 설정 파일

각 실행 구성은 JSON 설정 파일을 사용합니다.
...
SocketServer 설정은 여러 서버 인스턴스와 각 인스턴스의 포트 range를 저장할 수 있습니다.

```json
{
  "controlServers": [{ "host": "127.0.0.1", "port": 5000 }],
  "servers": [
    {
      "serverId": 1,
...
SocketClient 설정은 ControlServer endpoint를 여러 개 둘 수 있어 ControlServer 장애 시 fallback 구성이 가능합니다.
...
ControlServer, SocketServer, SocketClient는 기존 12바이트 프레임 위에 JSON payload 기반 control-plane 메시지를 사용합니다.
...
`SERVER_HEARTBEAT`에는 서버별 전체/현재/접속가능 수와 리소스 사용률이 포함됩니다.
...
대시보드는 cluster summary도 함께 표시하며 서버별 health, endpoint, 전체/현재/접속가능 클라이언트 수, CPU/MEM/STORAGE 사용률을 조회할 수 있습니다.
...
ControlServer 브로커를 통해 route를 받은 뒤 SocketServer로 직접 접속할 때:
...
테스트는 단일 `SocketTests` 프로젝트에 통합되어 있으며 MSTest를 사용합니다. `SocketCommon`, `SocketClient`, `SocketControl`, `SocketServer`, `SocketDashboard` 프로젝트를 참조해 공통 프로토콜, 클라이언트/서버 송수신, ControlServer 라우팅, 대시보드 상태 서비스를 함께 검증합니다. 대량 접속 검증은 별도 `SocketLoadTest` 프로젝트로 실행합니다.
...
| 라이브러리 | 버전 | 용도 | 라이선스 |
| --- | --- | --- | --- |
| `log4net` | `3.3.1` | 공통 로깅 | Apache-2.0 |
| `Google.Protobuf` | `3.20.1` | Protocol Buffers 지원 | BSD-3-Clause |
| `Grpc.Net.Client` | `2.46.0` | gRPC 클라이언트 지원 | Apache-2.0 |
| `Grpc.Tools` | `2.46.1` | gRPC/Protobuf 빌드 도구 | Apache-2.0 |
| `Microsoft.NET.Test.Sdk` | `17.2.0` | .NET 테스트 실행 | Microsoft Software License Terms (.NET Library) |
| `MSTest.TestAdapter` | `2.2.10` | MSTest 어댑터 | MIT |
| `MSTest.TestFramework` | `2.2.10` | MSTest 프레임워크 | MIT |
| `coverlet.collector` | `3.1.2` | 테스트 커버리지 수집 | MIT |
