...
static string RenderPage()
{
    ...
    html.AppendLine("<div style=\"grid-column:1/-1\"><label>Control Endpoints</label><textarea id=\"controlEndpoints\" placeholder=\"127.0.0.1:10000\"></textarea></div>");
    html.AppendLine("<div><label>Receive Timeout Seconds</label><input id=\"receiveTimeoutSeconds\" type=\"number\" min=\"1\"></div>");
    html.AppendLine("<div><label>Reconnect Retry Seconds</label><input id=\"reconnectRetrySeconds\" type=\"number\" min=\"1\"></div>");
    html.AppendLine("<div><label>Duplicate Backoff Seconds</label><input id=\"duplicateRejectBackoffSeconds\" type=\"number\" min=\"1\"></div>");
    ...
    html.AppendLine("function parseEndpoints(v){return v.split(/[\\n,]+/).map(x=>x.trim()).filter(Boolean).map(x=>{const i=x.lastIndexOf(':');return i>0?{host:x.substring(0,i),port:+x.substring(i+1)}:null}).filter(x=>x&&x.port>0)}");
    html.AppendLine("function formatEndpoints(v){return (v||[]).map(x=>`${x.host}:${x.port}`).join('\\n')}");
    html.AppendLine("function settings(){return {clientId:+el('clientId').value,clientName:el('clientName').value,host:el('host').value,port:+el('port').value,useControlServer:el('useControlServer').checked,controlEndpoints:parseEndpoints(el('controlEndpoints').value),receiveTimeoutSeconds:+el('receiveTimeoutSeconds').value,reconnectRetrySeconds:+el('reconnectRetrySeconds').value,duplicateRejectBackoffSeconds:+el('duplicateRejectBackoffSeconds').value,security:{profile:'EndToEndTls',transportMode:'Tls',tlsProtocol:'Tls13',requireTls13:true,requireClientCertificate:true,certificateDirectory:'',certificatePasswordEnvironmentVariable:'SOCKET_CERTIFICATE_PASSWORD',certificateRenewBeforeDays:30,rootCertificateLifetimeYears:10,moduleCertificateLifetimeYears:2,authenticationTimeoutMilliseconds:30000,messageEncryptionSecretEnvironmentVariable:'SOCKET_MESSAGE_SECRET',trustedNetwork:false},socketOptions:{connectTimeoutSeconds:30,readTimeoutSeconds:30,writeTimeoutSeconds:30}}}");
    html.AppendLine("function show(s){el('status').textContent=JSON.stringify(s,null,2)}");
    html.AppendLine("async function load(){const s=await json('/api/settings');el('clientId').value=s.clientId;el('clientName').value=s.clientName;el('host').value=s.host;el('port').value=s.port;el('controlEndpoints').value=formatEndpoints(s.controlEndpoints);el('useControlServer').checked=s.useControlServer;el('receiveTimeoutSeconds').value=s.receiveTimeoutSeconds;el('reconnectRetrySeconds').value=s.reconnectRetrySeconds;el('duplicateRejectBackoffSeconds').value=s.duplicateRejectBackoffSeconds;show(await json('/api/state'));}");
