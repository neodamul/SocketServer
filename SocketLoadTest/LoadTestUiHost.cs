...
internal static class LoadTestUiHost
{
    ...
    private static string RenderPage(LoadTestOptions options)
    {
        ...
        html.AppendLine("table{width:100%;border-collapse:collapse;font-size:13px}th{font-size:11px;text-transform:uppercase;letter-spacing:.6px;color:var(--muted2);text-align:left;padding:10px;border-bottom:1px solid var(--soft)}td{text-align:left;border-top:1px solid var(--soft);padding:10px;font-variant-numeric:tabular-nums}tbody tr[data-target]{cursor:pointer}tbody tr[data-target]:hover{background:rgba(255,255,255,.03)}tr.selrow{background:linear-gradient(90deg,rgba(91,140,255,.16),rgba(91,140,255,.02));box-shadow:inset 3px 0 0 var(--accent)}");
        ...
        html.AppendLine("let lastState=null;let selectedTarget=null;");
        html.AppendLine("function renderTargets(){const ts=lastState?lastState.targetServers:[];document.getElementById('targets').innerHTML=ts.map(t=>`<tr data-target=\"${t.targetServer}\" class=\"${String(t.targetServer)===String(selectedTarget)?'selrow':''}\"><td>${t.targetServer}</td><td>${t.connectedClients}</td></tr>`).join('');}");
        html.AppendLine("function renderClients(){const cs=lastState?lastState.clients:[];const rows=cs.filter(c=>String(c.targetServer)===String(selectedTarget)).sort((a,b)=>Number(a.clientId)-Number(b.clientId)||String(a.clientId).localeCompare(String(b.clientId),undefined,{numeric:true}));document.getElementById('clientsTable').innerHTML=rows.map(c=>`<tr><td>${c.clientId}</td><td>${c.isConnected}</td><td>${c.targetServer}</td></tr>`).join('');}");
        html.AppendLine("function render(s){lastState=s;document.getElementById('state').textContent=JSON.stringify(s,null,2);const c=s.counters;document.getElementById('metrics').innerHTML=metric('Status',s.status)+metric('Connected Now',s.connectedNow)+metric('Attempted',c.attempted)+metric('Connected Total',c.connected)+metric('Healthcheck OK',c.healthCheckSuccess)+metric('Failures',c.connectFail+c.healthCheckFail+c.registerFail+c.messageFail);const exists=s.targetServers.some(t=>String(t.targetServer)===String(selectedTarget));if(selectedTarget===null||!exists){selectedTarget=s.targetServers.length?s.targetServers[0].targetServer:null;}renderTargets();renderClients();}");
        html.AppendLine("document.getElementById('targets').addEventListener('click',function(e){const tr=e.target.closest('tr[data-target]');if(!tr)return;selectedTarget=tr.getAttribute('data-target');renderTargets();renderClients();});");
