...
public class TcpServer : SocketClient.Model.TcpClient, IServer, IClient, IDisposable
{
    ...
    {
        RelayLogger.Info($"Server relay received. receiverInstanceId={this.instanceId}, sourceServerId={relayMessage.SourceServerId}, sourceInstanceId={relayMessage.SourceInstanceId}, messageToken={relayMessage.MessageToken}, sourceClientId={relayMessage.SourceClientId}, targetClientId={relayMessage.TargetClientId}, ttlSeconds={relayMessage.TtlSeconds}");
        ...
        if (result.Success)
        {
            RelayLogger.Info($"Server relay delivered. receiverInstanceId={this.instanceId}, sourceServerId={relayMessage.SourceServerId}, sourceInstanceId={relayMessage.SourceInstanceId}, messageToken={request.MessageToken}, targetClientId={request.TargetClientId}");
        }
    }
    ...
    private async Task HandleServerRelayBatchAsync(ConnectionSession relaySession, ServerRelayBatchMessage relayBatch)
    {
        ...
        for (int itemIndex = 0; itemIndex < relayBatch.Items.Count; itemIndex++)
        {
            result.Items.Add(await this.ApplyServerRelayMessageAsync(relayBatch.Items[itemIndex], itemIndex));
        }
    ...
    private async Task<ServerRelayBatchResultItem> ApplyServerRelayMessageAsync(ServerRelayMessage relayMessage, int itemIndex = -1)
    {
        ClientMessageSendRequest request = CreateClientMessageRequest(relayMessage);
        ...
        if (DateTimeOffset.UtcNow - request.CreatedAt > TimeSpan.FromSeconds(Math.Max(1, request.TtlSeconds)))
        {
            ...
            RelayLogger.Warn($"Server relay expired. receiverInstanceId={this.instanceId}, sourceServerId={relayMessage.SourceServerId}, sourceInstanceId={relayMessage.SourceInstanceId}, messageToken={request.MessageToken}, targetClientId={request.TargetClientId}");
        ...
        if (!await this.DeliverToLocalClientAsync(request))
        {
            ...
            RelayLogger.Debug(() => $"Server relay target not connected. receiverInstanceId={this.instanceId}, sourceServerId={relayMessage.SourceServerId}, sourceInstanceId={relayMessage.SourceInstanceId}, messageToken={request.MessageToken}, targetClientId={request.TargetClientId}");
            return new ServerRelayBatchResultItem
            {
                ItemIndex = itemIndex,
                MessageToken = request.MessageToken,
    ...
    private async Task EnqueueServerRelayReceiveAsync(ConnectionSession relaySession, ServerRelayMessage relayMessage)
    {
        ...
        {
            RelayLogger.Warn($"Server relay request queue rejected receive command. receiverInstanceId={this.instanceId}, sourceServerId={relayMessage.SourceServerId}, sourceInstanceId={relayMessage.SourceInstanceId}, messageToken={relayMessage.MessageToken}");
    ...
    private async Task RunServerRelayRequestLoopAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                List<ServerRelayCommand> batch = null;
                try
                {
                    ...
                    batch = await this.CollectServerRelaySendBatchAsync(command, cancellationToken);
                ...
                catch (Exception exception)
                {
                    if (batch != null)
                    {
                        this.FailRelayCommands(batch, exception);
                    }
                    else
                    {
                        command.Completion.TrySetException(exception);
    ...
    private async Task ProcessServerRelayReceiveCommandAsync(ServerRelayCommand command)
    {
        ...
        {
            command.Completion.TrySetException(exception);
            RelayLogger.Warn($"Server relay receive command failed. instanceId={this.instanceId}", exception);
        }
    }
    ...
    private async Task SendServerRelayCommandBatchAsync(IReadOnlyList<ServerRelayCommand> commands)
    {
        foreach (IGrouping<string, ServerRelayCommand> group in commands.GroupBy(command => $"{command.Host}:{command.Port}"))
        {
            ...
            {
                this.FailRelayCommands(groupCommands, exception);
            }
        }
    }
        ...
        try
        {
            ...
            (bool success, SocketMessageFrame frame) = await channelPool.SendAndReceiveAsync(
                connection => ClientMessageProtocol.SendRelayAndReceiveAsync(
                    connection,
                    ClientMessageProtocol.CreateRelay(this.clusterId, this.instanceId, request, this.serverId)),
                SocketFactory.ReadTimeoutMilliseconds);
            if (!success)
            {
    ...
    private async Task SendRelayBatchToRemoteServerAsync(IReadOnlyList<ServerRelayCommand> commands)
    {
        ...
        ServerRelayBatchMessage batch = new()
        {
            CreatedAt = DateTimeOffset.UtcNow,
            Items = commands
                .Select(command => ClientMessageProtocol.CreateRelay(this.clusterId, this.instanceId, command.Request, this.serverId))
                .ToList()
        };
        ...
        try
        {
            ...
            {
                RelayLogger.Warn($"Server relay batch response invalid. sourceInstanceId={this.instanceId}, targetInstanceId={targetInstanceId}, endpoint={host}:{port}, count={commands.Count}, fallback=single");
                await this.SendRelayCommandsIndividuallyAsync(commands);
                return;
            }

            Dictionary<int, ServerRelayBatchResultItem> resultsByIndex = result.Items
                .Where(item => item.ItemIndex >= 0 && item.ItemIndex < commands.Count)
                .GroupBy(item => item.ItemIndex)
                .ToDictionary(group => group.Key, group => group.Last());
            for (int index = 0; index < commands.Count; index++)
            {
                ServerRelayCommand command = commands[index];
                if (command.Request == null ||
                    !resultsByIndex.TryGetValue(index, out ServerRelayBatchResultItem item))
                {
    ...
    private async Task SendRelayCommandsIndividuallyAsync(IReadOnlyList<ServerRelayCommand> commands)
    {
        foreach (ServerRelayCommand command in commands)
        {
            if (command.Request == null)
            {
                this.CompleteRelayCommand(command, false, command.TargetInstanceId, "Relay request was empty.");
                continue;
            }
    ...
    private void FailRelayCommands(IReadOnlyList<ServerRelayCommand> commands, Exception exception)
    {
        foreach (ServerRelayCommand command in commands)
        {
            command.Completion.TrySetException(exception);
