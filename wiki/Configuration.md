...
`requireClientCertificate=true`로 설정하면 mTLS를 사용하며, 클라이언트도 같은 Root CA로 서명된 모듈 인증서를 제시해야 합니다. `EndToEndTls` 인증서 검증은 Root CA 서명, SAN/name mismatch, serverAuth/clientAuth EKU를 확인합니다.
