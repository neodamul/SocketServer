...
`security.profile=EndToEndTls`는 앱까지 TLS를 유지하는 기본 profile입니다. `security.profile=EdgeTerminated`는 edge TLS 종단 이후 내부 신뢰망에서 앱 비-TLS 전송을 사용하는 profile이며 `trustedNetwork=true`가 필요합니다.
