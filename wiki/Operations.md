...
Route reservations are also transient. ControlServer keeps them in memory and relays them to peers, but excludes them from file persistence instead of writing the full registry file for every route request. Under broker-routed ramps, check nginx stream logs and ControlServer route latency together: long route stream times usually mean ControlServer command workers or registry writes are the bottleneck, not client healthcheck processing.
