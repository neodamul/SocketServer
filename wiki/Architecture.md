...
- **`EndToEndTls`** (default): keeps TLS to the app process; enables mTLS when `requireClientCertificate=true`; verifies server cert SAN/name, serverAuth/clientAuth EKU, and binds SocketClient cert SAN `socket-client-{clientId}` to the frame clientId when mTLS is enabled. For very large fan-out (e.g. 300k), place SocketServer nodes behind an L4 pass-through / TCP stream proxy and size shard count from measured per-node SslStream memory (see [Operations → Scale notes](Operations.md#scale-notes)).
