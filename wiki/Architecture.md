...
Dashboard는 설정된 ControlServer 목록을 조회하고 endpoint별 상태를 표시합니다. Healthy ControlServer가 있으면 해당 registry snapshot으로 전체 SocketServer 정보를 표시합니다. Dashboard 내부 서버 상태는 ControlServer 연결 여부와 무관하게 별도 그룹과 서버 목록에 항상 함께 표시됩니다. ControlServer가 없으면 로컬 Dashboard server 상태를 fallback cluster로 사용합니다.
