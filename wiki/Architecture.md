...
Client `ROUTE_REQUEST` frames can share a persistent ControlServer TLS channel. ControlServer sends `ROUTE_RESPONSE` and keeps the channel open for the next frame; clients that do not pool route lookups still close their short-lived route connection after receiving the response. `SocketLoadTest` uses a bounded `PersistentSecureChannelPool` for routed ramps unless strict per-client certificate binding is enabled, reducing repeated ControlServer TCP/TLS handshakes while preserving one route reservation per client. The client then opens its long-lived connection directly to the assigned SocketServer endpoint.
...
- `maxConnections`: 10,000 · `pendingAcceptCount`: 512 · `listenBacklog`: 4,096 · `idleTimeoutSeconds`: 90
